...
function detectModelFormat(url) {
  ...
  if (path.endsWith(".cif")) return "cif";
  if (path.endsWith(".pdb")) return "pdb";
  return "pdb";
}

function isValidStructure(text, format) {
  if (format === "cif") {
  ...
  useEffect(() => {
    ...
    async function initViewer() {
      try {
        ...
        if (PROJECT.modelUrl) {
          const modelFormat = detectModelFormat(PROJECT.modelUrl);
          ...
          if (!isValidStructure(modelText, modelFormat)) {
            throw new Error(`El archivo descargado no parece un ${modelFormat.toUpperCase()} válido.`);
          }

          viewer.addModel(modelText, modelFormat);
          applyStyles();
